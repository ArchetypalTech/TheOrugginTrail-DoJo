...
pub fn handle(ref world: IWorldDispatcher, player: Player, msg: Garble) {
    ...
    match msg.vrb {
        ...
        ActionType::Spawn => {
            let desc: ByteArray = lookat::describe_room_short(world_store, player.location.clone());
            let stub: ByteArray =
                "A thought has spawned in your mind, you can't quite make out what it is except ";
