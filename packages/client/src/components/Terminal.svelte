...
} from "$lib/stores/terminal_content_store";
...
async function submitForm(e: SubmitEvent) {
		...
		case "hear":
...

			{
				...
				if (target === "wind") {
					...
					addTerminalContent({
						text:
							state === "off" ? "Wind sound disabled" : "Wind sound enabled",
						format: "out",
						useTypewriter: false,
					});
				} else if (target === "tone") {
					...
					addTerminalContent({
						text:
							state === "off" ? "Tonal sound disabled" : "Tonal sound enabled",
						format: "out",
						useTypewriter: false,
					});
					...
					addTerminalContent({
						text:
							state === "off"
								? "Cricket sound disabled"
								: "Cricket sound enabled",
						format: "out",
						useTypewriter: false,
					});
				}
	...
	try {
		...
		if (get(connectedToArX) || get(connectedToCGC)) {
			...
			addTerminalContent({
				text: "shoggoth needs you to be in his realm...",
				format: "shog",
				useTypewriter: true,
			});
		}
...
>
  <div class="text-center">
    ...
    {/each}
    <div class="bastard">
      <b>B</b>est <b>A</b>rchetypal <b>S</b>ystem <b>T</b>erminals <b>A</b>nd
      <b>R</b>etrograde
      <b>D</b>evices
    </div>
  </div>
